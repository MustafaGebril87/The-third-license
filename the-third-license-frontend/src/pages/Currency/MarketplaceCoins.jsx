...
const ENDPOINTS = {
  ...
  BUY_SHARE_WITH_TOKENS: '/shares/buy',
  COIN_OFFERS: '/coin-market/offers',
  STRIPE_TOPUP_CREATE: '/stripe/topup/create',
};
...
const MarketplaceCoins = () => {
  ...
  const topUpWithStripeForOffer = async (offer) => {
    if (!offer?.id) return;
    ...
    try {
      ...
      const res = await axios.post(ENDPOINTS.STRIPE_TOPUP_CREATE, null, {
        params: { coinAmount: totalUsd, successUrl, cancelUrl },
      });
      ...
      if (!checkoutUrl) {
        ...
        setMessage('Failed: backend did not return checkoutUrl');
        return;
      }
      ...
      localStorage.removeItem('pendingTopUpOfferId');
      localStorage.removeItem('pendingTopUpUsd');
      setMessage('Failed to start Stripe checkout: ' + (err.response?.data || err.message || ''));
    } finally {
            ...
            {offers.map((offer) => {
              ...
              return (
                <li key={offer.id} className="border p-4 rounded shadow">
                  ...
                  <div><strong>Total:</strong> ${total.toFixed(2)} + 1% processing fee</div>
                  ...
                  <button
                    onClick={() => topUpWithStripeForOffer(offer)}
                  ...
                  >
                    {buyingOffer[offer.id] ? 'Redirecting...' : 'Top up wallet with Stripe'}
                  </button>
